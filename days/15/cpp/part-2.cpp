...
int main(int argc, char *argv[])
{
  ...
  for (int elf_attack_power = 4;; elf_attack_power++) {
    ...
    auto round = 0;
    bool an_elf_has_died = false;
    while (game_is_running and not an_elf_has_died) {
      ...
      for (auto& active_unit : units) {
        ...
        if (not std::any_of(units.cbegin(), units.cend(), 
          ...
          std::cout << battle_outcome << std::endl;
        ...
        if (targets_in_range) {
          ...
          if (targeted_foe->health <= 0) {
            ...
            if (not targeted_foe->is_goblin)
              an_elf_has_died = true;
          }
        }
      }
