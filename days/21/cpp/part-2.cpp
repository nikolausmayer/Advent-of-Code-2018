...
int main(int argc, char *argv[])
{
  ...
  std::set<I6> visited;
  std::map<long long, long long> steps_map;
  ...
  while (iptr >= 0 and iptr < (int)instructions.size()) {
    ...
    if (iptr == 28) {
      if (visited.contains(regs)) [[unlikely]]
        break;
      visited.insert(regs);
      if (steps_map[regs[5]] > 0) {
        steps_map[regs[5]] = std::min(steps_map[regs[5]], regs[5]);
      } else {
        steps_map[regs[5]] = icount;
      }
    }
