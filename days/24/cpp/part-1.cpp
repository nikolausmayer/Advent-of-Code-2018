...
int main(int argc, char *argv[])
{
  ...
  while (true) {
    ...
    //std::cout << attack_damage << " " << attack_type << " " << initiative << std::endl;
    //std::cout << std::endl;
  ...
  while (true) {
    ...
    for (auto idx : order) {
      ...
      for (auto& other : groups) {
        ...
        //std::cout << g.count << " deals " << damage << " to " << other.count << std::endl;
      }
      if (target != nullptr) {
        g.target = target;
        //std::cout << g.count << " selects " << target->count << std::endl;
    ...
    for (auto idx : order) {
      ...
      //std::cout << g.count << " attacks " << g.target->count << " for " << damage << " damage, killing " << dead << std::endl;
      g.target->count -= dead;
    }
